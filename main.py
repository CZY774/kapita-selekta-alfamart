...
def create_vendor():
    try:
        ...
        required_fields = ['vendorName', 'unitUsaha']
        for field in required_fields:
            if not data.get(field):
                return jsonify({"error": f"Missing required field: {field}"}), 400
        
        # Handle email validation
        email = data.get('emailCompany', '').strip() or None
        
        # Check for existing vendor with same email if email is not None
        if email:
            existing_vendor = vendors_collection.find_one({'emailCompany': email})
            if existing_vendor:
                return jsonify({"error": "A vendor with this email already exists"}), 400
        
        # Prepare vendor document
        vendor_doc = {
            '_id': data.get('_id') or generate_unique_id(),  # Use provided ID or generate new
            ...
            'emailCompany': email,  # Allow None if no email provided
        ...
        # Remove None values
        vendor_doc = {k: v for k, v in vendor_doc.items() if v is not None}
        ...
        app.logger.error(f"Vendor creation error: {str(e)}")
